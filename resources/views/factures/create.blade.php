            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        <x-lobiko.forms.input name="numero_facture" label="Numero (optionnel)" :value="old('numero_facture')" />
                    </div>
                    <div class="col-md-4 mb-3">
                        <x-lobiko.forms.input name="montant_final" type="number" step="0.01" label="Montant total" :value="old('montant_final')" required />
                    </div>
                    <div class="col-md-4 mb-3">
                        <x-lobiko.forms.select name="statut_paiement" label="Statut" :options="$statuts" :value="old('statut_paiement', 'en_attente')" />
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-4 mb-3">
                        <x-lobiko.forms.select name="type" label="Type" :options="$types" :value="old('type', 'consultation')" required />
                    </div>
                    <div class="col-md-4 mb-3">
                        <x-lobiko.forms.select name="nature" label="Nature" :options="$natures" :value="old('nature', 'normale')" required />
                    </div>
                    <div class="col-md-4 mb-3">
                        <x-lobiko.forms.input name="date_facture" type="date" label="Date de facture" :value="old('date_facture', now()->format('Y-m-d'))" required />
                    </div>
                </div>
                <div class="mb-3">
